...
- Return ONLY valid JSON"""
  ...
  {with_clause}
    ...
    def evaluate_response(self, task_input: str, output: str) -> int:
        ...
        try:
            prompt = f"""Evaluate the database optimization response on a 100-point scale.

Criteria:
1. DDL Quality (25 points): Structure, Iceberg optimizations, full paths
2. Migration Quality (25 points): Data transfer strategy, validation
3. Query Optimization (25 points): Performance improvements, correct paths
4. Execution Time (15 points): Expected performance gains
5. Storage Optimization (10 points): Compression, file sizes
...
Output: {output[:500]}..."""
            ...
            logger.warning("Не удалось получить оценку от LLM, используем 50")
            return 50
