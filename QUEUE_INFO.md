...
task_manager = SimpleTaskManager(
    max_workers=4,              # Максимум 4 задачи выполняются параллельно
...
| **max_workers** | **4** | Реально выполняются одновременно |
| **В ожидании** | **до 96** | Остальные ждут в очереди (100 - 4 = 96) |
...
│  🔄 Task #3 [RUNNING]   ← Выполняется (slot 1/4)     │
│  🔄 Task #4 [RUNNING]   ← Выполняется (slot 2/4)     │
│  🔄 Task #5 [RUNNING]   ← Выполняется (slot 3/4)     │
│  🔄 Task #6 [RUNNING]   ← Выполняется (slot 4/4)     │
│  ⏳ Task #7 [RUNNING]   ← Ждет семафор (в очереди)   │
│  ⏳ Task #8 [RUNNING]   ← Ждет семафор (в очереди)   │
│  ⏳ Task #9 [RUNNING]   ← Ждет семафор (в очереди)   │
│  ⏳ Task #10 [RUNNING]  ← Ждет семафор (в очереди)   │
└──────────────────────────────────────────────────────┘
...
Статус в /metrics:
{
  "tasks": {
    "total": 10,
    "running": 8,               ← Все с статусом RUNNING
    "actually_processing": 4,   ← Реально выполняются
    "queued": 4,                ← Ждут освобождения слота
    "completed": 2,
    "failed": 0
  },
  "queue": {
...
⚠️ **НЕ РЕКОМЕНДУЕТСЯ:** может привести к переполнению памяти!
...
| **Средняя нагрузка** ✅ | 4-6 | 100 | Рекомендуется (текущая) |
| **Высокая нагрузка** | 8-10 | 200 | Для продакшена с хорошим сервером |
| **Без ограничений** ⚠️ | 30 | 0 | Не рекомендуется |
...
- ✅ 100 задач → 4 выполняются, 96 ждут в очереди
...
Из них:
- **4 задачи** выполняются реально (через семафор)
- **до 96 задач** ждут в очереди
